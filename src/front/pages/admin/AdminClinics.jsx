...
export const AdminClinics = () => {
  ...
  // fin de los estados y comienzo de logica
  ...
  const executeToggle = async () => {
    try {
      ...
      const response = await fetch(
        backendUrl + `/api/clinics/${selectedClinic.id}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            reason: selectedClinic.is_active ? suspensionReason : null,
          }),
        },
      );
  ...
  const executeEdit = async (e) => {
    ...
    try {
      const backendUrl = import.meta.env.VITE_BACKEND_URL;
      const response = await fetch(
        backendUrl + `/api/clinics/${selectedClinic.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleToggleUserBan = async (clinicId, userId, newStatus) => {
    const isConfirmed = window.confirm(
      `¿Seguro que deseas ${newStatus ? "quitar el veto" : "vetar"} a este usuario?`,
    );
  ...
  return (
    <div
      className="container mt-5 py-4 flex-grow-1"
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            Administrador: <strong>{store.user?.email || "SuperAdmin"}</strong>
          </p>
        </div>
        <button
          ...
          <form onSubmit={handleCreateClinic} className="row g-3">
            ...
            <div className="col-md-6">
              ...
              <input
                ...
                onChange={(e) => handleChange(e)}
                placeholder="Ej: contacto@vetsalud.com"
              />
            </div>
            ...
            {filteredClinics.length > 0 ? (
              filteredClinics.map((clinic) => (
                <tr key={clinic.id}>
                  ...
                  <td>{clinic.rif}</td>
                  <td>
                    {clinic.telefono || <span className="text-muted">N/A</span>}
                  </td>
                  <td>
                    {clinic.correo || <span className="text-muted">N/A</span>}
                  </td>
                  <td>
                    {clinic.is_active ? (
                      ...
                      <div>
                        <span className="badge bg-danger mb-1">Suspendida</span>
                        <br />
                        <small
                  ...
                  <td>
                    <div className="d-flex justify-content-center gap-2">
                      <button
                        className={`btn btn-sm ${clinic.is_active ? "btn-outline-warning" : "btn-outline-success"}`}
                        onClick={() => {
                          setSelectedClinic(clinic);
